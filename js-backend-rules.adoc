...
You have to execute `docker-compose up -d --profile rel` in order to start the release versions of all services.

[CAUTION]
====
`docker-compose up` will *NOT* ensure that your release services images are up to date.
To ensure this you have to execute `docker-compose build --profile=rel` from the command line.
