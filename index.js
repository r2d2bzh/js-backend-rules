...
import { findDirectoriesWith, getProjectPath, spawn } from './utils.js';
...
const _install = async ({ logger, editWarning, serviceDirectories, subPackages, npmInstall = true }) => {
  await structureProject(logger);
  await tweakFiles({ logger, editWarning, serviceDirectories, subPackages });
  if (npmInstall) {
...
const tweakFiles = async ({ logger, editWarning, serviceDirectories, subPackages }) => {
